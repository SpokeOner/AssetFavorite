...
class RobloxClient:
    ...
    def _obtain_csrf_token(self, session: Session, user_id: str, asset_id: str = "1") -> Optional[str]:
        endpoints = [
            ...
            ("POST", f"https://catalog.roblox.com/v1/favorites/users/{user_id}/assets/1/favorite"),
    ...
    def _parse_error_reason(self, response_text: str, status_code: int) -> str:
        if not response_text:
            if status_code == 403:
                ...
                return "Asset not found or invalid endpoint"
        ...
        elif "purchasable" in response_lower or "sale" in response_lower:
            return "Asset not purchasable"
        
        return response_text[:60] if len(response_text) > 60 else response_text

    def execute_favorite(self, auth_cookie: str, proxy_addr: str, target_asset_id: str) -> Tuple[str, bool, str, int]:
    ...
    for auth_cookie in cookie_batch:
        ...
        while retry_count < max_retries and not succeeded:
            try:
                ...
                result_msg, success, response_data, status_code = client.execute_favorite(
                ...
                if success:
                    ...
                    print(f"{Fore.GREEN}Success{Style.RESET_ALL} | Asset: {asset_id} | Time: {request_time}s | Done: {completed} | {rate}/min")
                    succeeded = True
                else:
                    retry_count += 1
                    ...
                    print(f"{Fore.RED}Failed{Style.RESET_ALL} | Asset: {asset_id} | Try {retry_count}/{max_retries} | Status: {status_info} | Reason: {result_msg} | Failed: {failed} | {rate}/min")
                ...
                completed, failed, rate = tracker.get_stats()
                error_msg = str(e)[:40]
                print(f"{Fore.RED}Error{Style.RESET_ALL} | Asset: {asset_id} | Try {retry_count}/{max_retries} | {error_msg} | Failed: {failed} | {rate}/min")
                if retry_count < max_retries:
                    time.sleep(uniform(0.5, 1.5))
...
def main():
    ...
    try:
        ...
        target_asset = input("Enter asset id: ")
